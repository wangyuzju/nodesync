...
if(empty($pathto)){
	$pathto = '/home/bae/luoqin/nodesync';
}
...
//echo "strlen=".strlen(file_get_contents($_FILES["file"]["tmp_name"]));
//echojson('test ====== ', 22001);
*/
if($op && $pathto && $filepath){
	...
	echojson('First connect done or sync params incorrect :( ', 22001);
}
...
function exe_op($op){
	global $real_path, $pathto, $filepath,$fore_md5;
	switch($op){
		case 'mv':
