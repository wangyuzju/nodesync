...
startWatch = (opts)->
  ...
  ## for jch support and performance
  handleFileChange = ((enableJCH)->
    if enableJCH
      if opts.local
        ...
        handleFileChange = (e)->
          switch e.type
            when 'change'
              if path.basename(e.filename)[0..7] is "coffess_"
...
initMain = ()->
  program
    ...
    .option('', '')
    .option('-l, --local', 'disable auto sync feature for local testing')
    ...
    .version('0.0.25', '-v, --version')

  params = program.parse(process.argv)

  console.info "# enable [ local ] mode!" if params.local
