...
(function() {
  var Filter, MatchContainer, config, fc, fs, initMain, jch, parseArgv, path, program, remote, startWatch, watch;
  ...
  */
  ...
  initMain = function() {
    ...
    program.option('', '').option('resolve', 'resolve file conflict through svn').option('config', 'modify configuration file').option('', '').option('-p, --path <dir>', 'specifies dir path to be watched').option('-f, --force', 'force sync mode, without checking file\'s MD5').option('-h, --hidden', 'watch hidden files and dirs as well').option('-b, --beta', 'beta version, only stable on linux').option('-d, --debug', 'show more detailed debug info').version('0.0.16', '-v, --version');
